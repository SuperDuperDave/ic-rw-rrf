#!/usr/bin/env python3
"""Score the immutable cycle05 collection without any provider requests."""
import argparse
...
BASE = ROOT / 'results/cycle05-2026-09-10'
...
def score():
    ...
    for ordinal, record in enumerate(records, 1):
        ...
        if record['native_acceptable']:
            ...
            if record['p_positive'] is not None:
                ...
                predictions[pid] = Fraction(record['p_positive'])
            else:
                failures[pid] = 'parse:' + record['parse_failure']
        else:
            if record['p_positive'] is not None or ordinal != len(records):
    ...
    result = packets.score_predictions(fixture, predictions, failures)
