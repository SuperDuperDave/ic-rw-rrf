#!/usr/bin/env python3
"""Seal and collect one cycle11 four-packet verifier pilot; no retries or repair."""
import argparse
...
from evaluation import cycle11_certificates as packets
from evaluation import cycle11_verifier as measure
...
PROTOCOL = '_sessions/cycles/2026-09-11-cycle11-execution-protocol.md'
DECISION = '_sessions/evidence/2026-09-11-cycle11-execution-decision.json'
PINNED = {
    '_sessions/tools/tests/test_cycle09_runner.py': 'b75260d409c76f189515e882a0f3e3d5c3c385e01027ae4bc42dd34dfe0a24b2',
    '_sessions/tools/run_cycle09_verifier.py': '728668d998bc0d5d865218dea46b195ef3bb1ea42b8c3fa7ba9414341c215f10',
    '_sessions/tools/score_cycle09_verifier.py': '2d2233b76ed4056f00a5c7cd0b68fbbb151270cfc31063e7361c5f762ab09a74',
    'evaluation/cycle09_verifier.py': '5855ab8ebf9b6bfe4527d9e9c27109d2f7a22587b4f5ead6fb44f255080f4386',
...
def inspect_stream(raw, exit_code, expected_session, expected_ids, timed_out=False):
    """Native acceptance precedes strict terminal-only parsing of the answer and all four validity bits."""
    observed = native.inspect_stream(raw, exit_code, expected_session, timed_out)
    observed.pop('p_positive')
    observed.update(response=None, parse_failure=None)
    if observed['native_acceptable']:
        ...
        try:
            observed['response'] = measure.parse_response(terminal[0].get('result'), expected_ids)
...
def current_sources(prepared_manifest):
    extra = [Path(__file__), ROOT / 'evaluation/cycle11_verifier.py',
             ROOT / '_sessions/tools/tests/test_cycle11_runner.py',
             ROOT / '_sessions/tools/score_cycle11_verifier.py',
...
def prepare_execution(prepared_sha):
    ...
    if set(by_id) != set(measure.ORDER):
        raise ValueError('expected the four fixed packets')
...
def sources(execution_sha):
    ...
    expected_names = {'system-prompt.txt'} | {pid + '.json' for pid in measure.ORDER}
...
def collect(execution_sha):
    ...
    private = ROOT / '_sessions/local/cycle11' / batch_id
    ...
    try:
        for ordinal, packet_id in enumerate(fixture['request_order'], 1):
            ...
            record = {'ordinal': ordinal, 'packet_id': packet_id, 'session_id': session_id,
                ...
                **inspect_stream(stdout_path.read_bytes(), execution['exit_code'], session_id,
                    packets.parse_json(packet['payload'])['submission_ids'], execution['timed_out'])}
            ...
            print(json.dumps({'ordinal': ordinal, 'native_acceptable': record['native_acceptable'],
                              'valid_answer': record['response'] is not None,
                              'known_native_usd': str(spent), 'issues': record['issues']}), flush=True)
            if not record['native_acceptable'] or record['response'] is None or execution['interrupted']:
        ...
        manifest.update({'finished_utc': now(), 'wall_seconds': time.monotonic() - started,
            ...
            'valid_predictions': sum(r['response'] is not None for r in records),
