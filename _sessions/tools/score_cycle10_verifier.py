#!/usr/bin/env python3
"""Score the sealed cycle10 six-packet verifier collection; no provider calls."""
import argparse
    ...
    for ordinal, record in enumerate(records, 1):
        ...
        if record['native_acceptable']:
            ...
            if record['validity'] is not None:
                ...
                ids = packets.parse_json(by_id[pid]['payload'])['original_ids']
                predictions[pid] = measure.parse_validity(json.dumps(record['validity']), ids)
            else:
            ...
            if record['validity'] is not None or ordinal != len(records):
