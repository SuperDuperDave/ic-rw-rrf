#!/usr/bin/env python3
"""Score the sealed cycle11 four-packet verifier collection; no provider calls."""
import argparse
...
def score():
    ...
    if [r['packet_id'] for r in records] != list(measure.ORDER)[:len(records)]:
    ...
    for ordinal, record in enumerate(records, 1):
        ...
        if (record['ordinal'] != ordinal or record['payload_sha256'] != by_id[pid]['payload_sha256']
                or record['system_prompt_sha256'] != hashlib.sha256(measure.SYSTEM_PROMPT.encode()).hexdigest()):
        ...
        if record['native_acceptable']:
            ...
            if record['response'] is not None:
                ...
                ids = packets.parse_json(by_id[pid]['payload'])['submission_ids']
                predictions[pid] = measure.parse_response(json.dumps(record['response']), ids)
            else:
            ...
            if record['response'] is not None or ordinal != len(records):
    ...
    result = measure.score_decisions(fixture, predictions, failures)
    ...
    receipt.update({'status': 'complete', 'finished_utc': datetime.now(timezone.utc).isoformat(),
                    'measurement_status': result['status'], 'coverage': result['observed_counts'],
                    'output_sha256': {name: sha(output / name) for name in ('manifest-start.json', 'scores.json')}})
    write(output / 'manifest.json', receipt)
    print(json.dumps({'status': result['status'], 'coverage': result['observed_counts']}))
    return result
