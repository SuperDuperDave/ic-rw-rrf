#!/usr/bin/env python3
"""Seal and collect one cycle10 six-packet verifier pilot; no retries or repair."""
import argparse
...
from evaluation import cycle10_certificates as packets
from evaluation import cycle10_verifier as measure
...
PROTOCOL = '_sessions/cycles/2026-09-11-cycle10-execution-protocol.md'
DECISION = '_sessions/evidence/2026-09-11-cycle10-execution-decision.json'
PINNED = {
...
def current_sources(prepared_manifest):
    extra = [Path(__file__), ROOT / 'evaluation/cycle10_verifier.py',
             ROOT / '_sessions/tools/tests/test_cycle10_runner.py',
             ROOT / '_sessions/tools/score_cycle10_verifier.py',
...
def prepare_execution(prepared_sha):
    ...
    if set(by_id) != set(measure.ORDER):
        raise ValueError('expected the six fixed packets')
...
def collect(execution_sha):
    ...
    private = ROOT / '_sessions/local/cycle10' / batch_id
